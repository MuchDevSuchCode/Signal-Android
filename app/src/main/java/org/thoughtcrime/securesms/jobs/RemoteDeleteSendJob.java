...
public class RemoteDeleteSendJob extends BaseJob {
  ...
  private static final String KEY_TARGET_SENT_TIMESTAMP     = "target_sent_timestamp";
  ...
  /** Captured at creation time so the delete can still be sent after the local copy of the message has been removed. */
  ...
  private RemoteDeleteSendJob(long messageId,
                              ...
                              @Nullable RecipientId conversationRecipientId,
                              @NonNull Parameters parameters)
  {
    ...
    this.conversationRecipientId = conversationRecipientId;
  }
  ...
  public @Nullable byte[] serialize() {
    return new JsonJobData.Builder().putLong(KEY_MESSAGE_ID, messageId)
                                    ...
                                    .putLong(KEY_TARGET_SENT_TIMESTAMP, targetSentTimestamp)
                                    .putLong(KEY_CONVERSATION_RECIPIENT_ID, conversationRecipientId != null ? conversationRecipientId.toLong() : -1)
                                    .serialize();
  }
  ...
  protected void onRun() throws Exception {
    ...
    try {
      ...
      Log.i(TAG, "Local copy of the message has already been deleted. Continuing with data captured at creation time.");
    }
    ...
    if (message != null) {
      targetSentTimestamp     = message.getDateSent();
      conversationRecipientId = SignalDatabase.threads().getRecipientIdForThreadId(message.getThreadId());
      ...
      targetSentTimestamp     = this.targetSentTimestamp;
      conversationRecipientId = this.conversationRecipientId;

      if (targetSentTimestamp <= 0 || conversationRecipientId == null) {
        Log.w(TAG, "Message no longer exists and no creation-time data is available. Nothing to send.");
        return;
      }
    }
    ...
    boolean            isForStory         = message != null && message.isMms() && (((MmsMessageRecord) message).getStoryType().isStory() || ((MmsMessageRecord) message).getParentStoryId() != null);
    ...
    if (totalSkips.size() > 0 && conversationRecipient.getGroupId() != null) {
      SignalDatabase.groupReceipts().setSkipped(totalSkips, messageId);
    }

    if (recipients.isEmpty()) {
      if (message == null) {
        Log.i(TAG, "Delete sent. Local copy was already removed.");
      } else if (isForStory || !SignalStore.settings().isHideDeletedMessagesEnabled()) {
  ...
  public void onFailure() {
    ...
    try {
      ...
      if (!isForStory && SignalStore.settings().isHideDeletedMessagesEnabled()) {
        SignalDatabase.messages().deleteMessage(messageId);
      }
    } catch (NoSuchMessageException e) {
      // Local copy is already gone -- nothing to clean up
    }
  }
  ...
  public static class Factory implements Job.Factory<RemoteDeleteSendJob> {
    ...
    public @NonNull RemoteDeleteSendJob create(@NonNull Parameters parameters, @Nullable byte[] serializedData) {
      ...
      long              targetSentTimestamp        = data.getLongOrDefault(KEY_TARGET_SENT_TIMESTAMP, -1);
      long              conversationRecipientIdRaw = data.getLongOrDefault(KEY_CONVERSATION_RECIPIENT_ID, -1);
      RecipientId       conversationRecipientId    = conversationRecipientIdRaw > 0 ? RecipientId.from(conversationRecipientIdRaw) : null;

      return new RemoteDeleteSendJob(messageId, recipients, initialRecipientCount, targetSentTimestamp, conversationRecipientId, parameters);
    }
  }
}