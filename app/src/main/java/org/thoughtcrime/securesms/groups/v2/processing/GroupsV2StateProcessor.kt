    ...
    fun insertUpdateMessages(
      ...
      for (entry in processedLogEntries) {
        ...
        when {
          ...
          entry.change != null && changedFields.singleOrNull() == GroupChangeField.TIMER && SignalStore.settings.isHideExpireTimerUpdatesEnabled -> {
            Log.d(TAG, "Suppressing timer-only group change event.")
          }
