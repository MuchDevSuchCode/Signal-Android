...
public final class SettingsValues extends SignalStoreValues {
  ...
  /** When enabled (default), messages deleted for everyone are fully removed instead of leaving a "This message was deleted" tombstone. */
  public boolean isHideDeletedMessagesEnabled() {
    return getBoolean(HIDE_DELETED_MESSAGES, true);
  }

  public void setHideDeletedMessages(boolean enabled) {
    putBoolean(HIDE_DELETED_MESSAGES, enabled);
  }

  /** When enabled (default), "disappearing message timer changed" chat events are suppressed. */
  public boolean isHideExpireTimerUpdatesEnabled() {
    return getBoolean(HIDE_EXPIRE_TIMER_UPDATES, true);
  }

  public void setHideExpireTimerUpdates(boolean enabled) {
    putBoolean(HIDE_EXPIRE_TIMER_UPDATES, enabled);
  }

  /**
   * When enabled (default), you may delete an entire 1:1 chat from both devices, and you will honor
   * such a request from the other party. Disabling it both hides the menu option and makes incoming
   * requests be ignored.
   */
  public boolean isConversationDeleteForBothEnabled() {
    return getBoolean(CONVERSATION_DELETE_FOR_BOTH, true);
  }
