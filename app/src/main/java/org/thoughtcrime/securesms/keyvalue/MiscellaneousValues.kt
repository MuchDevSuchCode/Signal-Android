...
class MiscellaneousValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  companion object {
    ...
    /** Per-recipient watermark for the "delete conversation for both parties" feature. Suffixed with the recipient id. */
    private const val CONVERSATION_WIPE_PREFIX = "misc.conversation_wipe_time."
  }
  ...
  /**
   * Records that the 1:1 conversation with [recipientId] was wiped for both parties at [timestamp].
   * Messages sent before that point are dropped on arrival, so that messages already in flight when
   * the wipe happened don't resurrect the conversation.
   */
  fun setConversationWipeTime(recipientId: Long, timestamp: Long) {
    val key = "$CONVERSATION_WIPE_PREFIX$recipientId"
    if (timestamp > getLong(key, 0)) {
      putLong(key, timestamp)
    }
  }

  fun getConversationWipeTime(recipientId: Long): Long {
    return getLong("$CONVERSATION_WIPE_PREFIX$recipientId", 0)
  }
