...
public final class MessageRequestRepository {
  ...
  public @NonNull Single<Result<Unit, GroupChangeFailureReason>> deleteMessageRequest(@NonNull RecipientId recipientId, long threadId) {
    return deleteMessageRequest(recipientId, threadId, false);
  }

  public @NonNull Single<Result<Unit, GroupChangeFailureReason>> deleteMessageRequest(@NonNull RecipientId recipientId, long threadId, boolean alsoRemoteDelete) {
    //noinspection CodeBlock2Expr
    return Single.<Result<Unit, GroupChangeFailureReason>>create(emitter -> {
  ...
  public void deleteMessageRequest(@NonNull RecipientId recipientId,
                                   ...
                                   @NonNull GroupChangeErrorCallback error)
  {
    deleteMessageRequest(recipientId, threadId, false, onMessageRequestDeleted, error);
  }

  public void deleteMessageRequest(@NonNull RecipientId recipientId,
                                   long threadId,
                                   boolean alsoRemoteDelete,
                                   @NonNull Runnable onMessageRequestDeleted,
                                   @NonNull GroupChangeErrorCallback error)
  {
    executor.execute(() -> {
      ...
      if (alsoRemoteDelete) {
        sendRemoteDeleteForOutgoingMessages(threadId);
      }
  ...
  /**
   * Enqueues a "delete for everyone" for every outgoing message in the thread that is still within
   * the remote-delete window. The jobs capture what they need at creation time, so they will still
   * send after the local copies are removed by the thread deletion that follows.
   */
  @WorkerThread
  private void sendRemoteDeleteForOutgoingMessages(long threadId) {
    long now      = System.currentTimeMillis();
    int  enqueued = 0;

    try (MessageTable.MmsReader reader = MessageTable.mmsReaderFor(SignalDatabase.messages().getConversation(threadId))) {
      for (MessageRecord message : reader) {
        if (MessageConstraintsUtil.isValidRemoteDeleteSend(Collections.singleton(message), now)) {
          try {
            RemoteDeleteSendJob.create(message.getId()).enqueue();
            enqueued++;
          } catch (NoSuchMessageException e) {
            Log.w(TAG, "Unable to enqueue remote delete for message " + message.getId());
          }
        }
      }
    }

    Log.i(TAG, "Enqueued remote deletes for " + enqueued + " messages in thread " + threadId);
  }
