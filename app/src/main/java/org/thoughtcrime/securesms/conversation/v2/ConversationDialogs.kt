...
object ConversationDialogs {
  ...
  @JvmStatic
  /**
   * NON-UPSTREAM. Confirms deleting the entire 1:1 chat from both parties' devices.
   */
  fun displayDeleteForBothDialog(context: Context, recipient: Recipient, onDelete: () -> Unit) {
    MaterialAlertDialogBuilder(context)
      .setTitle(R.string.ConversationDialogs__delete_chat_for_both_title)
      .setMessage(context.getString(R.string.ConversationDialogs__delete_chat_for_both_body, recipient.getShortDisplayName(context)))
      .setPositiveButton(R.string.ConversationDialogs__delete_for_both) { _, _ -> onDelete() }
      .setNegativeButton(android.R.string.cancel, null)
      .show()
  }
