...
class MessageRequestViewModel(
  ...
  fun onDelete(alsoRemoteDelete: Boolean = false): Single<Result<Unit, GroupChangeFailureReason>> {
    return recipientId
      .flatMap { recipientId ->
        messageRequestRepository.deleteMessageRequest(recipientId, threadId, alsoRemoteDelete)
      }
      .observeOn(AndroidSchedulers.mainThread())
  }
