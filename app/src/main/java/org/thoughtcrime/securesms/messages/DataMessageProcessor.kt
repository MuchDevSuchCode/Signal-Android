...
object DataMessageProcessor {
  ...
  fun process(
    ...
    if (message.conversationDelete != null) {
      handleConversationDelete(context, envelope, message, senderRecipient, threadRecipient)
      return
    }

    if (isDroppedByConversationWipe(envelope, senderRecipient, threadRecipient)) {
      return
    }
  ...
  private fun handleExpirationUpdate(
    ...
    try {
      ...
      val insertResult: InsertResult? = if (SignalStore.settings.isHideExpireTimerUpdatesEnabled) {
        null
      } else {
  ...
  /**
   * NON-UPSTREAM. The other party asked us to delete our entire 1:1 conversation with them.
   *
   * We record a watermark at the requested time so that any of their messages that were already in
   * flight when they made the request are dropped on arrival rather than resurrecting the chat.
   */
  private fun handleConversationDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipient: Recipient, threadRecipient: Recipient) {
    log(envelope.clientTimestamp!!, "Conversation delete request from ${senderRecipient.id}")

    if (!SignalStore.settings.isConversationDeleteForBothEnabled) {
      warn(envelope.clientTimestamp!!, "[handleConversationDelete] Feature is disabled. Ignoring.")
      return
    }

    if (threadRecipient.isGroup || senderRecipient.isSelf) {
      warn(envelope.clientTimestamp!!, "[handleConversationDelete] Only valid for 1:1 chats from another party. Ignoring.")
      return
    }

    val requestedAtTimestamp: Long = message.conversationDelete!!.requestedAtTimestamp ?: envelope.clientTimestamp!!
    SignalStore.misc.setConversationWipeTime(senderRecipient.id.toLong(), requestedAtTimestamp)

    val threadId: Long? = SignalDatabase.threads.getThreadIdFor(senderRecipient.id)
    if (threadId == null) {
      log(envelope.clientTimestamp!!, "[handleConversationDelete] No thread to delete.")
      return
    }
    ...
    log(envelope.clientTimestamp!!, "[handleConversationDelete] Deleted thread $threadId at the request of ${senderRecipient.id}")
  }

  /**
   * NON-UPSTREAM. True if this message predates a "delete conversation for both parties" request
   * exchanged with this sender, meaning it was already in flight when the chat was wiped.
   */
  private fun isDroppedByConversationWipe(envelope: Envelope, senderRecipient: Recipient, threadRecipient: Recipient): Boolean {
    if (threadRecipient.isGroup) {
      return false
    }

    val wipeTime: Long = SignalStore.misc.getConversationWipeTime(senderRecipient.id.toLong())
    if (wipeTime <= 0) {
      return false
    }

    val sentTime: Long = envelope.clientTimestamp ?: return false
    if (sentTime >= wipeTime) {
      return false
    }

    warn(sentTime, "[conversationWipe] Dropping message from ${senderRecipient.id} that predates the conversation wipe at $wipeTime.")
    return true
  }
  ...
  fun handleRemoteDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipientId: RecipientId, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    return if (targetMessage != null && MessageConstraintsUtil.isValidRemoteDeleteReceive(targetMessage, senderRecipientId, envelope.serverTimestamp!!)) {
      ...
      } else if (SignalStore.settings.isHideDeletedMessagesEnabled) {
        val latestRevisionId = (targetMessage as? MmsMessageRecord)?.latestRevisionId?.id ?: targetMessage.id
        SignalDatabase.messages.deleteMessage(latestRevisionId)
      } else {
        SignalDatabase.messages.markAsRemoteDelete(targetMessage, senderRecipientId)
      }
  ...
  fun handleAdminRemoteDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipient: Recipient, threadRecipient: Recipient, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    return if (MessageConstraintsUtil.isValidAdminDeleteReceive(targetMessage, senderRecipient, envelope.serverTimestamp!!, groupRecord)) {
      if (SignalStore.settings.isHideDeletedMessagesEnabled) {
        val latestRevisionId = (targetMessage as? MmsMessageRecord)?.latestRevisionId?.id ?: targetMessage.id
        SignalDatabase.messages.deleteMessage(latestRevisionId)
      } else {
